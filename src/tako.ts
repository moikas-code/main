...
const TAKO = {
  ...
  getSellOrdersWithNFT: async ({
    ...
    var url =
      base +
      'v0.1/orders' +
      `/sell?blockchains=${blockchains.join(
        ','
      )}&origin=${origin}&continuation=${continuation}&size=${size}`;
    // console.log(url);
    let _data: any = await fetch(url, {
      method: 'GET',
    }).then((res) => res.json());
    ...
    for (const order of _data.orders) {
      const res = (await TAKO.get_item_by_nft_id(
        order.make.type.contract + ':' + order.make.type.tokenId
      )) as any;
      o[res.id] = {
        id: res.id,
        name: res.meta.name,
        ...
        content: res.meta.content.map((c: any) => {
          return {
            ...c,
            type: c['@type'],
          };
    ...
    return {
      continuation: _data.continuation || '',
      nfts: arr,
    };
  },
  get_all_items: async ({
  ...
  get_items_by_owner: async (address: any,blockchains:any,continuation: any
    ) => {
    try {
      const base = process.env.DEV !== 'true' ? baseURL : dev_baseURL
      // console.log(address)
      ...
      let url = (base +
        items +
        '/byOwner/' +
        `?owner=${address}&blockchains=${blockchains[0].toUpperCase()}&blockchains=${blockchains[1].toUpperCase()}&continuation=${continuation}`) as string;
      //
      let data = await fetch(url, {
        method: 'GET',
      }).then(async (res) => res.json());
